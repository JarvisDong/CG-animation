...
class Triangle {
    public:
        ...
        double Z[3];
        double colors[3][3];
        double normals[3][3]; /*Normals is indexed by the vertex first and the dimension second.
                                int vertexId = 0; int x = 0, y = 1, z =2; normals[vertexId][y] = … */
        double shading[3];
};
int ID = -1;
...
public:
    ...
    double          *depthBuffer;
    void fillColor(double color_rc[3], int c, int r, double z, double shading) {
        if (r >= height || c < 0 || c >= width || r < 0) { return; }
        int index = (r*width+c)*3;
        for (int i = 0; i < 3; i++) {
            buffer[index+i] = ceil_441(255*(fmin(1, color_rc[i]*shading)));
        }
    }
};
...
double norm(double vector[3]) { return sqrt(vector[0]*vector[0] + vector[1]*vector[1] + vector[2]*vector[2]); }

double dotProduct(double vector1[3], double vector2[3]) {
    return vector1[0]*vector2[0] + vector1[1]*vector2[1] + vector1[2]*vector2[2];
}

double CalculatePhongShading(LightingParameters &lp, double *viewDirection, Camera &c, double *normal) {
    double ambient, diffuse, specular, R[3];
    double normalized_normal[3];
//  ambient light
    ambient = lp.Ka;
//  diffuse light
    diffuse = lp.Kd*(fabs(dotProduct(lp.lightDir, normal))); // diffuse light = abs(L dot N)
//  specular light
    for (int i = 0; i < 3; i++) {
        normalized_normal[i] = normal[i]/norm(normal);
        R[i] = 2*(dotProduct(lp.lightDir, normal))*normalized_normal[i]-lp.lightDir[i]; // Calculate the largest proportion
        viewDirection[i] = c.position[i] - viewDirection[i];
    }
    double cosA = dotProduct(viewDirection, R)/(norm(viewDirection)*norm(R));
    specular = lp.Ks*(fmax(0, pow(cosA, lp.alpha))); // Ks * cosA^coefficient
//  shading amount
...
//  Initialize variables
    ...
    double shading_left = t.shading[0];
    double shading_right = t.shading[0];
    double shading_down = t.shading[0];
        ...
        for (int j = i+1; j < 3; j++) {
            if (t.Y[i] == t.Y[j]) {
                if (t.X[j] >= t.X[i]) {
                    ...
                    shading_left = t.shading[i];

                    x_right = t.X[j];
                    y_up_right = t.Y[j];
                    z_right = t.Z[j];
                    color_right = t.colors[j];
                    shading_right = t.shading[j];
                }
                else {
                    x_left = t.X[j];
                    y_up_left = t.Y[j];
                    ...
                    shading_left = t.shading[j];

                    x_right = t.X[i];
                    y_up_right = t.Y[i];
                    z_right = t.Z[i];
                    color_right = t.colors[i];
                    shading_right = t.shading[i];
                }
            }
        }
    }
...
//      Interpolate x(leftEnd) and x(rightEnd)
        double leftEnd = Lerp(r, x_left, x_down, y_up_left, y_down);
        double rightEnd = Lerp(r, x_right, x_down, y_up_right, y_down);
//      Interpolate z(leftEnd) and z(rightEnd)
        double Z_leftEnd = Lerp(r, z_down, z_left, y_up_left, y_down);
        double Z_rightEnd = Lerp(r, z_down, z_right, y_up_right, y_down);
//      Interpolate shading(leftEnd) and shading(rightEnd)
        double shading_leftEnd = Lerp(r, shading_left, shading_down, y_up_left, y_down);
        double shading_rightEnd = Lerp(r, shading_right, shading_down, y_up_right, y_down);
//      Interpolate Color(leftEnd) and Color(rightEnd)
...
//      Interpolate shading(leftEnd) and shading(rightEnd)
        double shading_leftEnd = Lerp(r, shading_up, shading_left, y_up, y_down_left);
        double shading_rightEnd = Lerp(r, shading_up, shading_right, y_up, y_down_right);
//      Interpolate Color(leftEnd) and Color(rightEnd)
        double color_leftEnd[3], color_rightEnd[3];
        for (int i = 0; i < 3; i++) {
...
//  relocate vertices
    ...
    double shading_up = t.shading[0];
    double shading_down = t.shading[0];
    double shading_mid = t.shading[0];
    ...
    double shading_mid_alt = Lerp(y_mid, shading_up, shading_down, y_up, y_down);
    ...
    /*     Determine all 3 vertices position for arbitrary triangle
       ...
       (x_mid, y_mid, z_mid, *color_mid)  *---------* (x_mid_alt, y_mid, z_mid_alt, *color_mid_alt)
                                        ...
                                        (x_down, y_down, z_down, *color_down)  
    */
// Split into 2 separate triangle
    Triangle top, bottom;
    top.X[0] = x_up;
    top.X[1] = x_mid;
    top.X[2] = x_mid_alt;
    ...
    top.Z[1] = z_mid;
    top.Z[2] = z_mid_alt;
    top.shading[0] = shading_up;
    top.shading[1] = shading_mid;
    top.shading[2] = shading_mid_alt;
    ...
    bottom.shading[0] = shading_down;
    bottom.shading[1] = shading_mid;
    bottom.shading[2] = shading_mid_alt;
    ...
    // compose 3 transforms into one. Finally we want CTxVTxDT
        ...
        */
        ...
        for (int i = 0; i < size; i++) {
            ...
            ID = i;
    // Calculate shading value for each vertices
            double viewDirectionA[3] = {t.X[0], t.Y[0], t.Z[0]};
            double viewDirectionB[3] = {t.X[1], t.Y[1], t.Z[1]};
            double viewDirectionC[3] = {t.X[2], t.Y[2], t.Z[2]};
            t.shading[0] = CalculatePhongShading(lp, viewDirectionA, c, t.normals[0]);
            t.shading[1] = CalculatePhongShading(lp, viewDirectionB, c, t.normals[1]);
            t.shading[2] = CalculatePhongShading(lp, viewDirectionC, c, t.normals[2]);
    // Transform vertices
            ...
            for (int j = 0; j < 3; j++) {
                ...
                T.shading[j] = t.shading[j];
            }
            // printf("Working on triangle %d\n", ID);
            // printf("Shading is %f, %f, %f\n\n", T.shading[0], T.shading[1], T.shading[2]);
    // Rasterize
            if (T.Y[0] == T.Y[1]){ 
